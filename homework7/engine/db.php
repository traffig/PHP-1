...
function getArrayImagesDb($sql = "SELECT * FROM `goods` ORDER BY `rating` DESC;")
{
...
{
    $dataDb = getDataDb("SELECT * FROM `feedback` WHERE `id_goods`='{$id}' ORDER BY `date` DESC;");
    $dataArray = [];
    while ($row = mysqli_fetch_assoc($dataDb)) {
        $dataArray[] = $row;
    }
...
{
    $dataDb = getDataDb("SELECT `id`, `name`, `image`, `description`, `rating` FROM `goods` WHERE `id`='$id';");
